# 🌿 Eixo Técnico

<p align="center">
  <img src="https://github.com/eixotecnico/eixotecnico/blob/main/images/tecnico.jpeg" width="850" alt="Eixo Técnico Banner">
</p>

<p align="center">
  <strong>Tecnologia, Sustentabilidade e Inovação para transformar comunidades.</strong>
</p>

<p align="center">
  Desenvolvimento de soluções inteligentes utilizando Robótica, Automação, Inteligência Artificial e Internet das Coisas (IoT).
</p>

---

## 🌎 Nossa Missão

O **Eixo Técnico** busca desenvolver e implementar projetos tecnológicos sustentáveis para comunidades cearenses, promovendo soluções renováveis, autônomas e acessíveis.

Acreditamos que a tecnologia deve gerar impacto positivo na sociedade, contribuindo para a preservação dos recursos naturais, a eficiência energética e a melhoria da qualidade de vida.

---

## 💚 Nossa Visão

Ser referência no desenvolvimento de tecnologias sustentáveis dentro da Universidade Federal do Ceará, conectando ensino, pesquisa e extensão para criar soluções que façam a diferença na sociedade.

---

## ⚙️ Áreas de Atuação

🌱 Sustentabilidade e Tecnologias Verdes

🤖 Robótica e Automação

📡 Internet das Coisas (IoT)

🧠 Inteligência Artificial

💻 Desenvolvimento Web e Mobile

🔬 Pesquisa e Desenvolvimento

⚡ Sistemas Embarcados

📊 Monitoramento e Análise de Dados

---

## 🚀 Projetos

### 🌱 Regador Automático

<p>Promover a integração entre saberes acadêmicos e conhecimentos tradicionais por meio do desenvolvimento e aplicação de tecnologias de automação voltadas para sistemas sustentáveis de irrigação e piscicultura no Sítio Wopila, em Maracanaú (CE), contribuindo para a valorização da agricultura familiar, o uso consciente da água e o fortalecimento da autonomia tecnológica de comunidades rurais.</p>

---

### ♻️ Máquina de Filamento

<p>O Projeto Máquina de Filamento é uma solução funcional, compacta e acessível para transformar garrafas PET em filamento para impressão 3D, com potencial de aplicação real em contextos de baixa infraestrutura. O sistema combina simplicidade mecânica com um controle eletrônico eficiente, o que permite uma operação fácil e a sua replicação em diferentes regiões. Dessa forma, a iniciativa contribui diretamente para a redução de resíduos plásticos e para a promoção da economia circular.</p>

---

### 🤖 Raitor

<p>O objetivo do Projeto Raitor (gerador solar), no contexto da ACCS RAITecS, é fornecer acesso à energia elétrica por meio de energia solar para o carregamento de equipamentos de baixa potência. Isso visa atender a necessidades básicas da comunidade de moradores de rua.</p>

---

## 🌍 Nosso Impacto

Os projetos desenvolvidos pelo Eixo Técnico possuem como objetivo:

- 💧 reduzir o desperdício de água;
- ♻️ incentivar a reciclagem e a economia circular;
- 🌱 promover práticas sustentáveis;
- ⚡ desenvolver soluções energeticamente eficientes;
- 🤝 contribuir com comunidades e projetos de extensão;
- 🎓 integrar ensino, pesquisa e inovação.

---

## 🌳 Nossos Valores

- Sustentabilidade
- Colaboração
- Ciência
- Tecnologia
- Inclusão
- Responsabilidade Social

---

## 🏛️ Sobre o RAITEC

O **Eixo Técnico** integra o **RAITEC — Robótica, Automação, Inteligência Artificial e Tecnologias**, grupo da **Universidade Federal do Ceará (UFC)** dedicado ao desenvolvimento de projetos de pesquisa, ensino e extensão.

Nossa atuação une estudantes de diferentes áreas para transformar ideias em soluções reais.

---

<p align="center">

### 🌿 Construindo tecnologias que cuidam do futuro.

**RAITEC • Universidade Federal do Ceará**

</p>
